...
/* ══ KALENDER AKTIVITAS (KLIK TANGGAL) ══ */
.grafik-row1{display:grid;grid-template-columns:1.1fr 1.4fr;gap:16px;align-items:start}
@media (max-width:960px){.grafik-row1{grid-template-columns:1fr}}
.kal-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}
.kal-nav-btn{
...
.kal-nav-btn:hover{background:var(--accent-light);border-color:var(--accent);color:var(--accent)}
.kal-title{font-family:'Outfit',sans-serif;font-weight:700;font-size:13px;color:var(--text)}
.kal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
.kal-dow{font-size:10px;font-weight:700;color:var(--text3);text-align:center;padding:4px 0;text-transform:uppercase}
.kal-day{
...
.kal-day:hover{border-color:var(--accent);background:var(--accent-light)}
.kal-day.empty{background:transparent;cursor:default;pointer-events:none}
.kal-day.today{border-color:var(--accent);font-weight:800;color:var(--accent)}
.kal-day.has-act{background:var(--accent-light2)}
.kal-day .dot-act{
    width:5px;height:5px;border-radius:50%;background:var(--accent);
    position:absolute;bottom:4px;
}
.kal-day.selected{background:var(--accent);color:#fff;border-color:var(--accent)}
.kal-day.selected .dot-act{background:#fff}
.kal-detail{
    margin-top:14px;padding:14px;border-radius:var(--r);background:var(--bg);
    border:1px solid var(--border);font-size:12.5px;color:var(--text2);min-height:74px;
}
.kal-detail .kal-detail-title{font-family:'Outfit',sans-serif;font-weight:700;color:var(--text);font-size:13px;margin-bottom:8px}
.kal-detail-row{display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px dashed var(--border)}
.kal-detail-row:last-child{border-bottom:none}
.kal-empty-msg{color:var(--text3);text-align:center;padding:14px 0}
</style>
<?php $__env->stopPush(); ?>
...
<div class="grafik-row1">
    ...
    <div class="chart-card">
        <div class="chart-title">
            <i class="fas fa-calendar-days" style="color:var(--accent);font-size:12px"></i>
            Kalender Aktivitas <span style="font-weight:500;color:var(--text3);font-size:11px">(klik tanggal untuk lihat detail)</span>
        </div>

        <div class="kal-head">
            <button type="button" class="kal-nav-btn" id="kalPrev"><i class="fas fa-chevron-left" style="font-size:10px"></i></button>
            <div class="kal-title" id="kalTitle">-</div>
            <button type="button" class="kal-nav-btn" id="kalNext"><i class="fas fa-chevron-right" style="font-size:10px"></i></button>
        </div>

        <div class="kal-grid" id="kalDow">
            <?php $__currentLoopData = ['Min','Sen','Sel','Rab','Kam','Jum','Sab']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="kal-dow"><?php echo e($d); ?></div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <div class="kal-grid" id="kalGrid" style="margin-top:4px"></div>

        <div class="kal-detail" id="kalDetail">
            <div class="kal-empty-msg">Klik salah satu tanggal di kalender untuk melihat ringkasan aktivitas & hasil kuis pada hari itu.</div>
        </div>
    </div>
...
// ══ KALENDER AKTIVITAS (KLIK TANGGAL) ══
(function() {
    const KAL_DATA_URL   = "<?php echo e(route('admin.dashboard.kalender-data')); ?>";
    const KAL_DETAIL_URL = "<?php echo e(route('admin.dashboard.kalender-hari', ':tgl')); ?>";
    ...
    const gridEl   = document.getElementById('kalGrid');
    const titleEl  = document.getElementById('kalTitle');
    const detailEl = document.getElementById('kalDetail');

    function pad(n){ return n < 10 ? '0'+n : ''+n; }
    function todayStr(){ return today.getFullYear()+'-'+pad(today.getMonth()+1)+'-'+pad(today.getDate()); }

    function renderCalendar(hariMap) {
        titleEl.textContent = BULAN_NAMA[viewMonth-1] + ' ' + viewYear;
        gridEl.innerHTML = '';

        const firstDow = new Date(viewYear, viewMonth-1, 1).getDay(); // 0=Min
        const daysInMonth = new Date(viewYear, viewMonth, 0).getDate();

        for (let i = 0; i < firstDow; i++) {
            const empty = document.createElement('div');
            empty.className = 'kal-day empty';
            gridEl.appendChild(empty);
        }

        for (let d = 1; d <= daysInMonth; d++) {
            const tgl = viewYear+'-'+pad(viewMonth)+'-'+pad(d);
            const cell = document.createElement('div');
            cell.className = 'kal-day';
            if (tgl === todayStr()) cell.classList.add('today');
            if (tgl === selectedDate) cell.classList.add('selected');
            const jml = hariMap[tgl] || 0;
            if (jml > 0) cell.classList.add('has-act');

            cell.innerHTML = d + (jml > 0 ? '<span class="dot-act"></span>' : '');
            cell.title = jml > 0 ? (jml + ' aktivitas') : 'Tidak ada aktivitas';
            cell.addEventListener('click', () => selectDate(tgl, cell));
    ...
    function loadMonth() {
        ...
        fetch(KAL_DATA_URL + '?bulan=' + viewMonth + '&tahun=' + viewYear)
            .then(r => r.json())
            .then(data => renderCalendar(data.hari || {}))
            .catch(() => { titleEl.textContent = BULAN_NAMA[viewMonth-1] + ' ' + viewYear; });
    }

    function selectDate(tgl, cellEl) {
        selectedDate = tgl;
        document.querySelectorAll('.kal-day').forEach(el => el.classList.remove('selected'));
        if (cellEl) cellEl.classList.add('selected');

        detailEl.innerHTML = '<div class="kal-empty-msg"><i class="fas fa-spinner fa-spin"></i> Memuat detail...</div>';

        fetch(KAL_DETAIL_URL.replace(':tgl', tgl))
            .then(r => r.json())
            .then(d => {
                let html = '<div class="kal-detail-title">📅 ' + d.tanggal + '</div>';
                if (d.total_aktivitas === 0 && d.total_kuis === 0) {
                    html += '<div class="kal-empty-msg">Tidak ada aktivitas belajar pada tanggal ini.</div>';
                } else {
                    html += '<div class="kal-detail-row"><span>👥 Siswa aktif</span><b style="margin-left:auto">' + d.siswa_unik + ' siswa</b></div>';
                    html += '<div class="kal-detail-row"><span>📖 Aktivitas belajar</span><b style="margin-left:auto">' + d.total_aktivitas + '</b></div>';
                    html += '<div class="kal-detail-row"><span>🏆 Kuis dikerjakan</span><b style="margin-left:auto">' + d.total_kuis + '</b></div>';
                    if (d.skor_rata !== null) {
                        html += '<div class="kal-detail-row"><span>⭐ Rata-rata skor</span><b style="margin-left:auto">' + d.skor_rata + '%</b></div>';
                    }
                }
                detailEl.innerHTML = html;
            })
            .catch(() => {
                detailEl.innerHTML = '<div class="kal-empty-msg">Gagal memuat detail tanggal ini.</div>';
            });
    }
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\final-fixed\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>