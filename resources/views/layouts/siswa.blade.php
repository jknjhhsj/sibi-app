...
<head>
    ...
    <title>@yield('title','SIBI Belajar')</title>
...
<div id="login-slide">
    ...
    <div class="ls-sheet">
        ...
        {{-- Errors --}}
        @if($errors->any())
        <div class="ls-err"><i class="fas fa-exclamation-circle"></i> {{ $errors->first() }}</div>
        @endif
        ...
        <div id="tab-masuk">
            <form action="{{ route('login') }}" method="POST">
                @csrf
                <input type="email" name="email" placeholder="Email" class="ls-input" value="{{ old('email') }}" required>
        ...
        <div id="tab-daftar" style="display:none">
            <form action="{{ route('register') }}" method="POST">
                @csrf
                <input type="text" name="name" placeholder="Nama lengkap" class="ls-input" value="{{ old('name') }}" required>
                <input type="text" name="kelas" placeholder="Kelas (opsional, cth: 5A)" class="ls-input" value="{{ old('kelas') }}">
                <input type="email" name="email" placeholder="Email" class="ls-input" value="{{ old('email') }}" required>
...
<header class="topbar">
    ...
    @auth
    <a href="{{ route('siswa.profil') }}" class="top-user-btn">
        <div class="top-av">{{ strtoupper(substr(Auth::user()->name,0,1)) }}</div>
        <span class="top-uname">{{ Auth::user()->name }}</span>
    </a>
    @else
...
<nav class="bottom-nav">
    ...
    @endphp
    @if($nav['route'] === 'siswa.dashboard' && !Auth::check())
        <button onclick="openLogin()" class="nav-item {{ $isActive ? 'active':'' }}">
            <div class="nav-icon-wrap"><span class="nav-ico">{{ $nav['ico'] }}</span></div>
            <span class="nav-lbl">{{ $nav['lbl'] }}</span>
        </button>
    @else
        <a href="{{ $url }}" class="nav-item {{ $isActive ? 'active':'' }}">
            <div class="nav-icon-wrap"><span class="nav-ico">{{ $nav['ico'] }}</span></div>
            <span class="nav-lbl">{{ $nav['lbl'] }}</span>
        </a>
    @endif
    @endforeach
    @auth
    <form action="{{ route('logout') }}" method="POST" style="flex:1;display:contents">
        @csrf
...
@if($errors->has('email') || $errors->any()) openLogin(); @endif
